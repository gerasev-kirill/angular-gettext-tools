    ...
    Extractor.prototype.extractJs = function (filename, src, lineNumber) {
        ...
        walkJs(syntax, function (node, parentComment) {
            ...
            if (isGettext(node) || isGetString(node)) {
                ...
                gettextFinders = self.options.customJSGettextFinders || [];
                for (var i = 0; i < gettextFinders.length; i++) {
                    if (!gettextFinders[i].isGettext && !gettextFinders[i].getJSData){ continue; }
                    if (gettextFinders[i].isGettext(node)){
                        finderResult = gettextFinders[i].getJSData(node);
                        if (finderResult.singular || finderResult.plural){
                            singular = finderResult.singular;
                            plural = finderResult.plural;
                            context = finderResult.context;
                        }
                    }
                }
            }
